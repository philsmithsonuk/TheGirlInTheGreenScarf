...
function showProgressAnimation(){
    ...
    */
    /** Added by Awesemo
     * Custom effect on shop bag status
     */
	if($('header-bag-status').hasClassName('show')){
		$('header-bag-items').hide();
	}
    ($('header-bag-status').select('span')[0]).innerHTML = $('updating-cart-status').innerHTML;
    addEffectACP($('header-bag-status').select('span')[0], 'blink');
}
